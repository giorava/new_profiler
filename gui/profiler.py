...
class EnvironmentVariableGUI:
    def __init__(self, root):
        ...
        self.expID_entry  = self.create_entry('Experiment ID', 1)
        self.number_of_images_entry  = self.create_entry('Number of images', 2)
        self.dw_iterations_entry  = self.create_entry('Number of dw iterations', 3)
        self.dapi_channel_name_entry  = self.create_entry('Dapi channel name', 4)
        self.yfish_channel_name_entry  = self.create_entry('YFISH channel name', 5)
        self.pixel_dx_entry = self.create_entry('Pixel dx', 6)
        self.pixel_dy_entry = self.create_entry('Pixel dy', 7)
        self.pixel_dz_entry = self.create_entry('Pixel dz', 8)
        self.estimated_nuc_diameter_entry  = self.create_entry('Estimate nuclei diameter', 9)
        ...
        self.preprocessing_estimated_time_entry.insert(0, "12:00:00")
        ...
        self.memory_per_image_entry.insert(0, "60GB")
        self.threads_entry = self.show_advance_options('Number of Threads', 14)
        self.threads_entry.insert(0, 10)
        self.segmentation_estimated_time_entry = self.show_advance_options('segmentation_estimated_time', 15)
        self.segmentation_estimated_time_entry.insert(0, "12:00:00")
        self.use_dw_dapi_entry = self.show_advance_options('use_dw_dapi', 16)
        self.use_dw_dapi_entry.insert(0, True)
    ...
    def save_to_config(self):
        ...
        config['EnvironmentVariables'] = {
            ...
            'use_dw_dapi': self.use_dw_dapi_entry.get()            
        }
    ...
    def profiler(self): 
        print("submitting profiler")  
        
    def segmentation(self): 
        print("submitting segmentation") 
        
    def clean_folders_preproc(self): 
        print("Clean after preprocessing")  
        
    def plot_fovs(self): 
        print("plotting Fovs")
        
    def after_run_cleaning(self): 
        print("After Run cleaning")
    ...
    def load_config(self):
        if os.path.exists(self.config_file):
            ...
            if 'EnvironmentVariables' in config:
                ...
                if "use_dw_dapi" in variables:
                    self.use_dw_dapi_entry.delete(0, tk.END)
                    self.use_dw_dapi_entry.insert(0, variables['use_dw_dapi'])
