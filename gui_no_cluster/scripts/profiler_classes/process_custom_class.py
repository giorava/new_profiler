...
class ProcessCustom(): 
    ...
    def __init__(self, path_raw_image: str, dw_iterations: str, threads: str, perform_decolvolution: str, path_to_bin: str):
        ...
        with open(f"{path_to_output}/conversion_{slideID}_{fov_name}.log", "w") as f: 
            ...
            if fov_index == None: 
                ...
                for index, output in enumerate(output_ch_list):
                    conv_pipe.append(f"{self.path_to_bin}/bftools/bfconvert -channel {index} {path_to_image} '{output}'")
            else: 
                conv_pipe = []
                for index, output in enumerate(output_ch_list):
                    conv_pipe.append(f"{self.path_to_bin}/bftools/bfconvert -series {fov_index} -channel {index} {path_to_image} '{output}'")
    ...
    def deconvolution(self, slideID:str, imageID:str, fov_name:str, path_to_output:str, output_ch_list:"list[str]",
        ...
        with open(f"{path_to_output}/PSF_{slideID}_{fov_name}.log", "w") as f: 
            ...
            for emission, output in zip(ch_lambdas_em, output_ch_list):
                psf_pipe.append(f"{self.path_to_bin}/dw_bw --threads {threads} --NA {NA} --lambda {emission} --ni {ni} --resxy {dxy} --resz {dz} --verbose 2 {path_to_output}/PSF_{NA}_{ni}_{emission}_{dxy}_{dz}.tiff")
        ...
        with open(f"{path_to_output}/dw_{slideID}_{fov_name}.log", "w") as f: 
            ...
            dw_pipe = []
            for emission, output in zip(ch_lambdas_em, output_ch_list): 
                dw_pipe.append(f"{self.path_to_bin}/dw --iter {dw_iterations} --threads {threads} '{output}' {path_to_output}/PSF_{NA}_{ni}_{emission}_{dxy}_{dz}.tiff")       
