    ...
    command = f"sbatch --partition=cpuq --mem=36GB --job-name='p{expID}' \
                    --time='{segmentation_estimated_time}' \
                        --output={path_raw_folder}/{expID}_profile.log \
                            --export=expID={expID},path_raw_folder='{path_raw_folder}',dapi_channel_name='{dapi_channel_name}',yfish_channel_name='{yfish_channel_name}',pixel_dx='{pixel_dx}',pixel_dy='{pixel_dy}',pixel_dz='{pixel_dz}',deconvolved_for_profile='{deconvolved_for_profile}' \
                                scripts/sbatch_scripts/profiles.sh"
